...
class ChildReferral(Document):
	...
	if TYPE_CHECKING:
		from frappe.apf.doctype.any_medical_compliction.any_medical_compliction import AnyMedicalCompliction
		from frappe.core.doctype.diagnosis_child_table.diagnosis_child_table import Diagnosischildtable
		from frappe.core.doctype.referral_child_table.referral_child_table import Referralchildtable
		from frappe.core.doctype.treatment_child_table.treatment_child_table import Treatmentchildtable
		from frappe.core.doctype.treatment_nrc_child_table.treatment_nrc_child_table import TreatmentNRCchildtable
		...
		child_referral_hi: DF.Data | None
		child_referral_od: DF.Data | None
		child_status: DF.Link
		...
		discharge_date: DF.Date | None
		district_id: DF.Link
		gp_id: DF.Link
		height_on_referral: DF.Float
		medical_compliction_options: DF.TableMultiSelect[AnyMedicalCompliction]
		medical_compliction_other: DF.Data | None
		name: DF.Int | None
		partner_id: DF.Link
		referral_reason: DF.TableMultiSelect[Referralchildtable]
		referred_to: DF.Link | None
