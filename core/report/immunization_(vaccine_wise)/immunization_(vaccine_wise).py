...
def get_columns():
    columns = [
        ...
        {"label": "Eligible Enrolled Children (active)", "fieldname": "active_children", "fieldtype": "Data", "width": 250},
        {"label": "No of Children vaccinated", "fieldname": "fv", "fieldtype": "Data", "width": 250},
        {"label": "Percentage %", "fieldname": "percentage", "fieldtype": "Data", "width": 250},
    ]
    return columns
...
def get_summary_data(filters=None):
    ...
    sql_query = f"""
        SELECT
            ...
            SUM(
                CASE 
                    WHEN (SELECT COUNT(*) FROM `tabVaccine Details` AS vd WHERE vd.parent = att.name ) 
                    THEN 1 
                    ELSE 0 
                END
            ) AS fv,
(SUM(
                CASE 
                    ...
                    ELSE 0 
                END
            ) /(SELECT COUNT(*) FROM `tabChild Profile` AS cp WHERE cp.creche_id = c.name AND cp.is_active = 1) ) *100 AS percentage
